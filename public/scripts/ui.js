class PeersUI {
  ...
  async _onKeyDown(e) {
    ...
    // close About FIleOnGO page on Escape
    if (e.key === "Escape") {
...
class Dialog {
  ...
  hide() {
    ...
    document.title =
      "FileOnGo | Transfer Files to Any Devices. No Setup, No Signup.";
  ...
  async _displayFiles(
    ...
    if (files.length > 1) {
      ...
      try {
        ...
        filenameDownload = `FIleOnGO_files_${year + month + date}_${
          hours + minutes
        }.zip`;
      } catch (e) {
        console.error(e);
...
class ReceiveRequestDialog extends ReceiveDialog {
  ...
  _showRequestDialog(request, peerId) {
    ...
    document.title = `${transferRequestTitle} - FIleOnGO`;
    changeFavicon("images/favicon-96x96-notification.png");
...
class PublicRoomDialog extends Dialog {
  constructor() {
    ...
    this.$footerBadgePublicRoomDevices = $$(
      ".discovery-wrapper .badge-room-public-id"
    );
    ...
    this.$headerBtnJoinPublicRoom.addEventListener("click", (_) =>
      this._onHeaderBtnClick()
    );
    this.$footerBadgePublicRoomDevices.addEventListener("click", (_) =>
      this._onHeaderBtnClick()
    );
